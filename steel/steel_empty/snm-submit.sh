...
module load openmpi
mpirun -np $SLURM_NTASKS singularity exec --bind ${BIND_PATH}:${BIND_PATH} frensie_hpc.simg python snm.py --sim_name="snm" --num_particles=1e12 --threads=$SLURM_CPUS_PER_TASK --db_path=${BIND_PATH}/database.xml
